...
class VocalTestState extends State<VocalTestPage> {
  ...
  MultiVerseTestForm? _findFromCollection(){
    ...
    while(mvrsMatch.isNotEmpty){
      for(int i = int.parse(mvrsMatch[0][0].toString()); i<int.parse(mvrsMatch[1][0].toString());i++){
        lst.add(i);
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: (state==Answer.pasiv)?Colors.grey.shade900:((state==Answer.right)?Colors.green.shade300:Colors.red.shade300),
      body: (_loaded)
          ? LayoutBuilder(
                      ...
                      children: [
                        (_list.isNotEmpty)?Align(
                          ...
                          children: [
                            ...
                            ...List.generate(_rigt.length, (index) => Text(_rigt[index].multiVerse.getShortName(),style: const TextStyle(color:Colors.white),)),
                            const SizedBox(height: 20,),
                            Align(
                              alignment: Alignment.center,
                              child: Text("오답 ${_rigt.length}회"),
                            ),
                            const SizedBox(height: 10,),
                                              ...
                                              setState(() {
                                                ...
                                                if(_spoken.contains("아멘")){
                                                  ...
                                                  if(search!=null) {
                                                    ...
                                                    final submit = tmp.substring(tmp.indexOf("말씀입니다")+"말씀입니다".length).replaceAll("아멘", "");
                                                    final answer = search.getAllVerses();
                                                    if(submit==answer){
